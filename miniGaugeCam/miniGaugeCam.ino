...
/**/       const float GCVersion=0.55;     //version of software to be printed in log. lets user know whether curren or not from log
...
if (timeToNextSample>12){              //If it is going to be a while take a long nap
     BlinkOutput(1,100,100);               //Give a little feedback
      LowPower.powerDown(SLEEP_8S, ADC_OFF, BOD_OFF);//Put Arduino into lower power state for 8 seconds
     //LowPower.idle(SLEEP_8S, ADC_OFF, TIMER2_OFF, TIMER1_OFF, TIMER0_OFF, SPI_OFF, USART0_OFF, TWI_OFF);
     // delay(8000);
    }
      
else if (timeToNextSample>5){          //If is going to be a short while, take a short nap
     BlinkOutput(1,100,100);
//delay(1000);     //Give a little feedback
     LowPower.powerDown(SLEEP_4S, ADC_OFF, BOD_OFF); //Put Arduino into lower power state for 4 seconds
     //LowPower.idle(SLEEP_4S, ADC_OFF, TIMER2_OFF, TIMER1_OFF, TIMER0_OFF, SPI_OFF, USART0_OFF, TWI_OFF);
      }
      
else if(timeToNextSample<5){//If the time to the next picture is very short, give some visual feedback, and take a picture
      long int countdown=millis();
      while(millis()-countdown<timeToNextSample*1000){//Delay until time has arrived
         if (millis()%100==0){                    //every 100ms make a flash
                  BlinkOutput(1,10,10);
                  }//end if(millis()...
         ...
         delay(1000);                              //Wait for camera to do any adjusting
...
//Open a file, and determine the size of the file to store
      File imgFile = SD.open(filename, FILE_WRITE);        // Open the file for writing
